...
def compare_to_groundtruth_fus(gt, dt, dist='iou', distfields=None, distth=0.5):
    """Compare groundtruth and detector results.
    ...
    for fid in allframeids:
        ...
        if fid in fid_to_fdt:
            fdt = fid_to_fdt[fid]
            hids = fdt.index.get_level_values('Id')
        dists = np.zeros((len(oids),len(hids)))
        if len(oids) > 0 and len(hids) > 0:
            for i in range(len(oids)):
                for j in range(len(hids)):
                    if oids.values[i] == hids.values[j]:
                        dists[i][j] = compute_dist([fgt.values[i]], [fdt.values[j]])
                    else:
                        dists[i][j] = np.nan
        acc.update(oids, hids, dists, frameid=fid)
        # print(dists)
